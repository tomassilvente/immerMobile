...
export default function top100(): JSX.Element {
  ...
  const toggleState = useCallback((key: 'shareOpen' | 'play' | 'shuffle' | 'replay') => {
    ...
    <MobileLayout>
      <div className="h-screen"
        ...
        <div className="flex mt-[50px]">
          {
            state.shuffle
              ? <p onClick={() => toggleState("shuffle")} className="ml-[60px] w-[30px] mt-[12px] text-[24px] text-primary"> 1.5x </p>
              : <p onClick={() => toggleState("shuffle")} className="ml-[60px] w-[30px] mt-[12px] text-[24px] text-white"> 1x </p>
          }
          <p className="absolute ml-[145px] text-white font-light mt-[19px]">30</p>
          <SvgBack30 width={50} height={60} className="ml-[40px] mt-[2px]" />
          {
            state.play
              ? <SvgPauseOrange onClick={() => toggleState("play")} width={70} height={65} className="ml-[25px]" />
              : <SvgPlayButtonOrange onClick={() => toggleState("play")} width={70} height={65} className="ml-[25px]" />
          }
          <p className="absolute ml-[316px] text-white font-light mt-[21px]">30</p>
          <SvgFoward30 width={50} height={60} className="ml-[25px] mt-1" />
          {
            state.replay
              ? <SvgSleepOrange onClick={() => toggleState("replay")} width={22} height={64} className="ml-[50px]" />
              : <SvgSleep onClick={() => toggleState("replay")} width={22} height={64} className="ml-[50px]" />
          }
        </div>
        <div className="grid grid-cols-12 mt-[50px]">
          <SvgShareCirclesWhite onClick={() => toggleState("shareOpen")} width={30} height={30} className="col-start-9 ml-[10px]" />
      ...
      {state.shareOpen && (
        ...
        >
          <Share openShare={state.shareOpen} />
        </div>
      )}
    </MobileLayout>
  )
