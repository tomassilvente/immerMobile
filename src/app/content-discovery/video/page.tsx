...
export default function content(): JSX.Element {
    ...
    const toggleLike = useCallback(() => {
        setState(prevState => ({ ...prevState, isLiked: !prevState.isLiked }))
    }, [])

    const toggleShare = useCallback(() => {
        setState(prevState => ({ ...prevState, isShareOpen: !prevState.isShareOpen }))
    }, [])

    const toggleComments = useCallback(() => {
        setState(prevState => ({ ...prevState, isCommentOpen: !prevState.isCommentOpen }))
    }, [])

    return (
            <MobileLayout >
                <EventHeader shareOpen={state.isShareOpen} openShare={toggleShare} eventImg={'../../assets/piano.jpeg'} />
                <main className="m-4">
                    ...
                    <div className="flex mt-4 text-center font-light">
                        ...
                        <div className="ml-10" onClick={toggleLike}>
                            {
                                state.isLiked
                                    ? <>
                        ...
                        <div className="ml-10" >
                            <SvgCommentButton onClick={toggleComments} width={40} height={40} />
                            <p>93</p>
                        </div>
                        <div onClick={toggleBookmark} className="ml-[200px] mt-1" >
                            {
                                state.isBookmarked
                ...
                {state.isShareOpen && (
                    ...
                    >
                        <Share openShare={state.isShareOpen} />
                    </div>
                )}
                {state.isCommentOpen && (
                    <div
                        className="bg-[#000000d8]"
                    >
                        <Comments openComments={state.isCommentOpen} />
                    </div>
                )}
            </MobileLayout>
    )
