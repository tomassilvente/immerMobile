...
export default function EventHeader({ eventImg, setShareOpen }: any) {
  return (
    <>
      <div
        className="h-[40%] m-5 grid grid-cols-12 place-items-end content-start "
        style={{
      ...
      >
        ...
        <p className="text-center font-semibold text-xl col-start-4 col-end-9 mb-3 mr-6">
          {DemoData.eventName}
        </p>
        <SvgLikeButton
        ...
        <div className="col-start-2 mt-[375%]">
          {DemoData.DemoTicketLocation.map((ticket) => (
            <div className="flex ml-14" key={ticket.Location}>
              <div
                className="h-[10px] w-[20px] mb-3"
                style={{ backgroundColor: ticket.Color }}
              ></div>
              <p className="ml-1 text-xs">{ticket.Location}</p>
            </div>
          ))}
        </div>
      </div>
